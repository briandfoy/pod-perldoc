...
[B<-X>] PageName|ModuleName|ProgramName

B<perldoc> B<-f> BuiltinFunction

B<perldoc> B<-q> FAQ Keyword

See below for more switches described.
...
=item B<-d> F<destination-filename>
...
=item B<-X> use an index if it is present

The B<-X> option looks for an entry whose basename matches the name
given on the command line in the file C<$Config{archlib}/pod.idx>. The
